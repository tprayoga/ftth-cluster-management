...
export const RoleSwitcher: React.FC = () => {
  const { currentUser, availableUsers, setCurrentUserRole, logout } = useCluster();
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="hidden sm:block text-xs leading-tight">
          <span className="font-bold text-slate-900 dark:text-white truncate max-w-[110px] block">
      ...
      {/* DROPDOWN MENU */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 rounded-2xl glass-card border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl p-2.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150 text-xs">
          {/* USER HEADER CARD */}
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 mb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span
                  className={`inline-block px-1.5 py-0.2 text-[9px] font-extrabold rounded border mt-1 ${getRoleBadgeColor(
                    currentUser.role
                  )}`}
                >
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-2 pt-2 border-t border-slate-200 dark:border-slate-700/50 leading-relaxed">
          ...
          <div className="mb-2">
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span>Hak Akses ({currentUser.permissions?.length || 0} Izin)</span>
              </div>
              <ChevronDown className={`w-3.5 h-3.5 transition-transform ${showRoleInfo ? 'rotate-180' : ''}`} />
            </button>

            {showRoleInfo && (
              <div className="mt-1 p-2 rounded-lg bg-slate-100/70 dark:bg-slate-800/40 space-y-1">
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-2 mb-2">
            <span className="px-2 text-[10px] uppercase font-bold tracking-wider text-slate-400 block mb-1.5">
              Ganti Akun / Role Cepat
            </span>
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <strong className="text-slate-900 dark:text-white font-bold block text-[11px] truncate">
                          {user.name}
                        </strong>
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-2">
            <button
              ...
              className="w-full flex items-center gap-2 p-2 rounded-xl text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 font-bold transition-colors text-xs"
            >
